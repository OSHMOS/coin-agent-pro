...
const inOpenRouter = document.getElementById('openrouter_key');
...
async function init() {
  ...
  if (config) {
    ...
    inOpenRouter.value = config.OPENROUTER_API_KEY || config.GLM_API_KEY || config.GROQ_API_KEY || config.GEMINI_API_KEY || '';
...
btnSave.addEventListener('click', async () => {
  ...
  const newConfig = {
    ...
    OPENROUTER_API_KEY: inOpenRouter.value,
